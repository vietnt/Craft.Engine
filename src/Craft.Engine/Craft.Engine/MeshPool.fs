...
let private tryFindFreeSlot (pool: MeshPool) =
    ...
    let rec f c =
        ...
        if checkSlot id then ValueSome id
        elif c > 0 then f (c - 1)
        else ValueNone
...
let add (vertex: _ []) (index: int []) offset vcount icount (pool: MeshPool) =
    ...
    let box = Box(vertexPos, offset, 0, vertexPos + vcount*pool.stride, 1, 1)
    ctx.UpdateSubresource (vertex, pool.vertexBuffer, 0, pool.stride, 0, Nullable(box))
    // update index buffer
    let box = Box(indexPos, offset, 0, indexPos + icount*4, 1, 1)
